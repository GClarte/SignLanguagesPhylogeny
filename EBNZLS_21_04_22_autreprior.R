...
J=read.csv("datasets/datafincorr.csv",header=T,stringsAsFactors =T, na.strings="")
...
#Toutes langues
#quelleslangues=c("Lithuanian","Czech","German","French","Russian","AmericanEnglish","Italian","Austrian","Ukrainian","Spanish","Polish",
#                 "English","NewZealand","Estonian","Latvian","Chinese","Hongkong","Taiwan","Japanese","AllMissing")
...
Pri=list(c(1,1000,0,1),c(1,1000,0,1),c(1,1000,0,1),c(1,1000,0,1),c(1,1000,0,1))
...
pribruit=list(list(function(){runif(1,10^-5,10^-3)},function(x){
  if(x>10^-5 & x<10^-3){
    return(-log(x))
  } else {
    return(-Inf)
...
save.image(paste("EuropeBNZ_21_04_22_autreprior", date(), ".RData"), version = 2)
