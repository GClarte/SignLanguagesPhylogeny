# This script runs the analysis to reconstruct a phylogeny of all sign languages (European, New Zealand, Asian).
# As noted in the supplementary materials, the reconstructed phylogenies should NOT be trusted from this analysis.
...
priorcoal <- function(tr, agemax) {
  tip = Prior$tipprior
  if (!all(sapply(tipprior, function(tip) {
    is.monophyletic.perso(statet$Tr, tip)
  }))) {
    return(-Inf)
  } else {
    L = sum(tr$edge.length)
    n = length(tr$tip.label)
    return(log((1 - exp(-partopo * L / 2)) ^ (n - 1) * (exp(partopo * t /
                                                              2))))
  }
}

etats = sapply(1:nch, function(x) {
  length(levels(J[, qui[x]]))
})
...
priorbeta <-
  function(x, y) {
    if (length(x$root[[2]]) > 0) {
      ...
      dbeta((sum(x$edge.length) - 2 * y) / ((length(x$tip.label) - 2) * y), 1, 100, log =
              T)
    }
  }
...
Prior = list(
  ...
  pribruit = pribruit,
  #prior sur le bruit avec tout défini
  tipprior = list(1:15, 16:19, 1:19)
)
#tipprior=list())
Param = list(
...
save.image(paste("ToutesLS", date(), ".RData"), version = 2)
