...
gibbstopopart2 <-  function(tr,
  ...
  if( qqqq==1){
    ...
    if (runif(1,0,1)>1/2 || length(tr$root[[2]])>1){
      ...
      if (length(agemax)==1){
        ...
        VV = modificationtopooutgroupsansresc_agevar(tr,
                                              X ,
                                              NL,
                                              L ,
                                              Tps,
  ...
  if (alphabis==-Inf || VV[[9]]==-1){
    ...
    alph = lkldtopotout(trt, Xt, la, NLt, rho, Lt,P) - lkldtopotout(tr, X, la, NL, rho, L,P) + alphabis
...
modificationtopooutgroupsansresc <-
  function(tr,
    ...
    if (runif(1)<1/2){ #on transforme en outgroup
      ...
      hautmax=min(trt$edge.length[bbb],agemax-hauteur(tr,e))#minimum length at the root, that we split in two
      if(hautmax<0){
        return(list(tr, X, NL, L, Tps, 0,M,loiappliste,-1,-1,rep(-1,nch),La,Rho,bruit))
      }
      ...
      haut=hauteur(trt,e)
      hauts=hauteurtous(trt,feuilles,agemax)
      quiposs=which(hauts[[2]]>haut &!(1:max(tr$edge))%in%c(e,tr$root[[2]],f,tr$edge[st,2])) 
      ...
      for (ii in 1:nch) {
          ...
          corrb = corrb - sum(log(P[[ii]][Xt[[ii]][[i]]])) - dpois(length(Xt[[ii]][[i]]), La[ii] *
           ...
           corrb = corrb + sum(log(P[[ii]][X[[ii]][[i]]])) + dpois(length(X[[ii]][[i]]), La[ii] *
                                                                       tr$edge.length[i], log = T)
          #resc=resc+jacrescale*(nch-2)*log(trt$edge.length[i]/tr$edge.length[i])
        Xt[[ii]][[j]] = sample(1:length(P[[ii]]),
                               rpois(1, La[ii] * trt$edge.length[j]),
                               prob = P[[ii]],
                               replace = T)
      ...
      at=f
      trc=trt
      zt=trt$edge[trt$edge[,2]==at,1]#nœud parent de a
      ...
      trc$edge.length[hkt]=trt$edge.length[hkt]-hautt
      ...
      haut_e=hauteur(tr,e)
      hauts=hauteurtous(tr,feuilles,agemax)
      quiposs=which(hauts[[2]]>haut_e & !(1:max(tr$edge))%in%c(e,tr$root[[2]],f,tr$edge[st,2])) 
      if (length(tr$edge[,1]==f)==0 || length(quiposs)<1){
        return(list(tr, X, NL, L, Tps, 0,M,loiappliste,-1,-1,rep(-1,nch),La,Rho,bruit))
      } else {
        a=quiposs[sample(1:length(quiposs),1)]
        ...
        for (ii in 1:nch){
          ...
          trc$edge[jt,]=c(dt,gt)
          trc$edge[ifrt,]=c(gt,ft)
          trc$edge.length[ifrt]=trt$edge.length[jt]+trt$edge.length[ifrt]
          bbb=which(trt$edge[,1]==root[[2]])
          hautmax=min(trt$edge.length[bbb],agemax-hauteur(trt,e))#minimum length at the root, that we split in two
          trc$edge.length[jt]=runif(1,0,hautmax)
          corrb=corrb-log(1/hautmax)
      }
...
modificationtopooutgroupsansresc_agevar <-
  function(tr,
           X,
           NL,
           L,
    ...
    if (runif(1)<1/2){ #on transforme en outgroup
      ...
      corrb=corrb+log(1/(hauteur(tr,g)-max(hauteur(tr,e),hauteur(tr,f))))
      bbb=which(trt$edge[,1]==root[[2]])
      hautplus=rgamma(1,0,10/hauteur(tr,tr$root[[2]]))
      trt$edge.length[j]=hautplus
      corrb=corrb-dgamma(hautplus,0,10/hauteur(tr,tr$root[[2]]),log=T)
      trt$edge.length[i]=hautplus+hauteur(tr,root[[2]])-hauteur(tr,e)
      trt$root[[2]]=d
      #corrb=corrb+log(1/(hauteur(trt,g)-max(hauteur(trt,e),hauteur(trt,f))))#legth of the possible branh inside
      haut=hauteur(trt,e)
      hauts=hauteurtous(trt,feuilles,agemax)
      ...
      haut_e=hauteur(tr,e)
      hauts=hauteurtous(tr,feuilles,agemax)
      corrb=corrb+dgamma(min(tr$edge.length[aaa]),1,10/hauteur(tr,f),log=T)
      quiposs=which(hauts[[2]]>haut_e & !(1:max(tr$edge))%in%c(e,tr$root[[2]],f,tr$edge[st,2])) 
      if (length(tr$edge[,1]==f)==0 || length(quiposs)<1){
        return(list(tr, X, NL, L, Tps, 0,M,loiappliste,-1,-1,rep(-1,nch),La,Rho,bruit))
      } else {
