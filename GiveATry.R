...
priorunif <- function(tr,agemax){
  ...
  if (!all(sapply(tips,function(tip){is.monophyletic.perso(tr,tip)})) ||
      !all(sapply(cladeage,function(clade){ageconstraints(tr,clade)}))){#tests for the constrains on clades and on internal ages
    return(-Inf)
  } else {
    hhh=hauteur(tr,tr$root[[2]])
    if (!(hhh>200&hhh<1000)){#add here the constrains on the root age.
      return(-Inf)
    } else {
...
Pri=lapply(1:nch,function(x){c(1,500,0,1)})
...
pribruit=list(list(function(){runif(1,10^-6,10^-2)},function(x){ #compute the value of the prior
  if(x>10^-6 & x<10^-2){
    return(-log(x))
  } else {
    return(-Inf)
...
list(function(x){rnorm(1,x,.1)}, #an idoneous proposal
     function(x,y){dnorm(x,y,.1,log=T)}))


#we put everythin in two nice lists
#in Prior all the prior informations
Prior=list(Prirho=Prirho,#prior on Rho
           hyperpbini=hyperpbini,#prior on the transf choice parameter
           Prila=Pri,#prior on the transf rate
           priortree=priorunif,#prior on the tree
           pribeta=matrix(rep(c(10,1),nch),ncol=nch,nrow=2), #prior on beta the acceptance rate, here Beta(10,1)
           #bruittemp=0.05-log(1:30)*(.05-.0005)/log(30),
           bruittemp=c(.1,.05,.03,.02,.01,.009,.008,.007,.006,.005,.001),#decreasing scheme for the noise, the final value must be in a non-zero area of the function pribruit
           pribruit=pribruit,#prior sur le bruit avec tout défini
           tipprior=list())#here you can include several vectors indicating the indices of the leaves that must form a clade
           #eg : tipprior=list(c(1,2,3),c(6,7))
           #tipprior=list())
#
  Param=list(Trpossliste=passages,#possible transformations
           nch=nch,nph=etats,
           npart=3000,#number of particles. BEWARE, large value (over 4000 in my case) can cause memory overload, and R does not warn, see later
           npas=5000,#number of step between each of the change in the noise value
           npasfin=10000,#we finish with a very long MCMC
           Nmin=2500,#if the ISS is smaller than Nmin, we resample, in practice we always resample
           loiini=loiini,
           Prob=c(1,.01,.1,.5,1,0,.01),#probability of the proposals in the MCMC when the noise is still fixes
           ...
           Probfin=c(1,.01,.1,.5,1,.1,.01),#when the noise can change Prob[7]!=0
           agemax=c(100,1/3),#there are two possibilities, first you can fix it, giving a single number,
           ...
           tiplabel=quelleslangues,#label of the tips IN THE SAME ORDER THAN IN THE DATASET
           batches=c(lapply(1:(nlangues),function(x){ncogntot})), #don't change that, even if it's not used anymore
           ReconstructClade=list(),#list of vector of indices of the leaves whose common ancestor state must be reconstructed
