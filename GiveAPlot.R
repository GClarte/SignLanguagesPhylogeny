...
lat=data.frame(value=c(lat),character=c(t(matrix(c("handshape","location","# of hands","movement"),nrow=length(VV2[[1]][[1]][[2]]),ncol=npart))))
...
h
ggsave(paste("lambdas_",name,".pdf",sep=""),height=5,width=8,unit="cm")
...
h=ggplot(data=data.frame(value=sapply(VV2[[1]],function(x){x[[4]]})),aes(x=value))+geom_density()+
  theme(axis.title.y=element_blank(),
        axis.text.y=element_blank(),
        axis.ticks.y=element_blank()) + labs(x=expression(mu))
...
betat=data.frame(value=c(betat),character=c(t(matrix(c("handshape","location","# of hands","movement"),nrow=length(VV2[[1]][[1]][[2]]),ncol=npart))))
...
h
ggsave(paste("beta_",name,".pdf",sep=""),height=5,width=8,unit="cm")
...
for (ch in 1:nch){
  ...
  ggsave(paste("transformations_ch",as.character(ch),name,".png",sep=""), height=6,width=10,unit="cm")
}
#==========================================================================
#                       ROOT AGE
#==========================================================================
h=ggplot(data=data.frame(value=sapply(VV2[[1]],function(x){hauteur(x[[1]],x[[1]]$root)})),aes(x=value))+geom_density()+
  theme(axis.title.y=element_blank(),
        axis.text.y=element_blank(),
        axis.ticks.y=element_blank())+xlim(0,600)
...
for (Lg1 in 1:(length(quelleslangues2)-1)){
  for (Lg2 in (1+Lg1):length(quelleslangues2)){
    ...
    iii=iii+1
    nomscolonnes=c(nomscolonnes,paste(quelleslangues2[Lg1],"_",quelleslangues2[Lg2],sep=""))
    #h=ggplot(data=datacogn,aes(x=Nb,y=Posterior))+geom_point() +geom_text(aes(label=levels(J[,3])),hjust=0, vjust=0)
    if(simu){
      h=ggplot(data=datacogn,aes(x=Nb,y=Posterior,color=vrai))+geom_point() +geom_text(aes(label=Meaning),hjust=0, vjust=0)+
        scale_colour_discrete("True cognacy status")
...
UU=lapply(VV2[[1]],function(x){
  ...
  A1=Reduce('c',lapply(y[[1]],function(z){x$X[[ch]][[z]]}))
  A2=Reduce('c',lapply(y[[2]],function(z){x$X[[ch]][[z]]}))
  return(list(A1,A2))
})

HH=Reduce('c',sapply(UU,function(x){x[[1]]}))
JJ=Reduce('c',sapply(UU,function(x){x[[2]]}))

transfnoms=sapply(Param$Trpossliste[[1]],function(x){paste(levels(J[,qui[1]])[x[1]],">",levels(J[,qui[1]])[x[2]])})

h=ggplot(data=data.frame(Number=as.vector(table(factor(HH,levels=1:length(Param$Trpossliste[[ch]]))))/npart,Transformation=transfnoms),
         aes(x=Transformation,y=Number)) +
  geom_bar(stat="identity") + theme(axis.text.x = element_text(angle = 70, hjust=1))
h

g=ggplot(data=data.frame(Number=as.vector(table(factor(JJ,levels=1:length(Param$Trpossliste[[ch]]))))/npart,Transformation=transfnoms),
         aes(x=Transformation,y=Number)) +
  geom_bar(stat="identity") + theme(axis.text.x = element_text(angle = 70, hjust=1))
g

ggsave(paste("frequency_transformations",Param$tiplabel[Lg1],"_nca(",Param$tiplabel[Lg1],Param$tiplabel[Lg2],")_ch",ch,".pdf"),height=15,width=20,unit="cm")


HH=Reduce('c',sapply(UU,function(x){x[[2]]}))
...
ggsave(paste("frequency_transformations",Param$tiplabel[Lg2],"_nca(",Param$tiplabel[Lg1],Param$tiplabel[Lg2],")_ch",ch,".pdf"),height=15,width=20,unit="cm")
...
UU=sapply(1:npart, function(x){rowSums(VV2[[1]][[x]][[9]][,VV2[[1]][[x]]$Tr$edge[,1]==VV2[[1]][[x]]$Tr$root])==0})
datacogn <- data.frame(Posterior=rowMeans(UU),Nb=1:nrow(Dat[[1]]),Meaning=quelmeanings)
datacogn$Meaning=factor(quelmeanings,levels=as.character(quelmeanings))
h=ggplot(data=datacogn,aes(x=Nb,y=Posterior))+geom_point() +geom_text(aes(label=levels(J[,3])),hjust=0, vjust=0)
if(simu){
...
h
ggsave("cognats_European-Asian_families.pdf",width=17,height=10,unit="cm")
...
for (l1 in 1:nlang){
  for (l2 in (l1+1):nlang){
    L1=Lgf1[l1]
    L2=Lgf1[l2]
    UU=sapply(1:npart, function(x){rowSums(VV2[[1]][[x]][[9]][,trouvercheminaux(VV2[[1]][[x]][[1]],L1,L2,numeric())])==0})
    datacogn <- data.frame(Posterior=rowMeans(UU),Nb=1:nrow(Dat[[1]]),Meaning=quelmeanings)
    datacogn$Meaning=factor(quelmeanings,levels=as.character(quelmeanings))
    #h=ggplot(data=datacogn,aes(x=Nb,y=Posterior))+geom_point() +geom_text(aes(label=levels(J[,3])),hjust=0, vjust=0)
    #if(simu){
    ...
    jj=jj+1
    nomsfamille=c(nomsfamille,paste(quelleslangues[L1],"_",quelleslangues[L2],sep=""))
  }
}
...
mean(sapply(VV2[[1]],function(x){sum(x$NL)}))

#total mean number of changes for the first character due to transformations
mean(sapply(VV2[[1]],function(x){sum(sapply(x$X[[4]],length))*x$beta[1]*nrow(Dat[[1]])}))

#total mean number of changes dues to the noise
mean(sapply(VV2[[1]],function(x){nrow(Dat[[4]])*x$noise*x$Tr$edge.length}))

#mean number of new cognacy apparition per meaning on the tree
mean(sapply(VV2[[1]],function(x){sum(x$Tr$edge.length)*x$Rho}))
